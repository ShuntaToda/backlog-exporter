...
import {BODY_END_MARKER, BODY_START_MARKER} from '../domain/markdown/body-marker.js'
import {BacklogHttpClient} from '../infrastructure/backlog/http-client.js'
import {stubLogger} from '../testing/stub-logger.js'
...
describe('exportDocuments', () => {
  ...
  it('documentIdsで指定したドキュメントのみ保存されること', async () => {
    ...
    nock(`https://${DOMAIN}`).get('/api/v2/documents/da').query({apiKey: API_KEY}).reply(200, documentDetail('da', 'docA', 'docAの本文'))
