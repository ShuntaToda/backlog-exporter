...
backlog-exporter/1.1.1 linux-x64 node-v22.23.2
...
- Wiki の本文は Backlog の原文のまま維持されます（添付参照記法の書き換えは行いません）。添付ファイルへは `## 添付ファイル` セクションのリンクからアクセスできます
- ドキュメントの本文は Backlog の構造化データ（JSON）から Markdown に変換されます（見出し・リスト・表・コードブロック等を再現します）。構造化データを取得できない場合はプレーンテキストの本文をそのまま使用します。添付参照記法の書き換えは行わないため、添付ファイルへは `## 添付ファイル` セクションのリンクからアクセスできます
...
<!-- backlog-exporter:body:start -->
ここにドキュメントの本文内容が入ります。
Backlog の構造化データから変換された Markdown が入ります。
<!-- backlog-exporter:body:end -->
