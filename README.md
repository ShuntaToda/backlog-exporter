...
**抽出時の規約**: 抽出ツールは「ファイル内で**最初に現れる開始マーカー**から、**その後最初に現れる終了マーカー**まで」を本文とみなしてください。課題ファイルでは終了マーカーの後ろに任意のユーザーテキスト（コメント）が続くため、「最後の終了マーカー」を基準にするとコメントがマーカー文字列を引用しただけで範囲が壊れます。この規約の制約として、**本文自体に終了マーカーの文字列そのものを書くと抽出はそこで打ち切られます**（マーカー文字列を本文に書かない運用としてください）。

**既存エクスポートへの注意**: マーカーが付くのは、この機能の導入後にエクスポート・再取得されたファイルだけです。`update` はBacklog側で更新があったものしか再取得しないため、既存のファイルにマーカーを一括で付与したい場合は再エクスポートしてください。
